    ...
    <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-1">
            @foreach(['/' => 'Home', '/pricing' => 'Harga', '#fitur' => 'Fitur', '/login' => 'Masuk'] as $url => $label)
                <a href="{{ $url }}" class="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors">{{ $label }}</a>
            @endforeach
        </nav>

        <div class="hidden md:flex items-center gap-2">
            <a href="/login" class="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors">Masuk</a>
            <a href="/register" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors">Coba Gratis</a>
        </div>
    ...
    <div x-show="open" x-cloak
         ...
         class="md:hidden border-t border-gray-100 bg-white px-6 py-4 space-y-1">
            @foreach(['/' => 'Home', '/pricing' => 'Harga', '#fitur' => 'Fitur'] as $url => $label)
                <a href="{{ $url }}" class="block px-3 py-2 text-sm font-medium text-gray-600 hover:text-gray-900 rounded-lg hover:bg-gray-50">{{ $label }}</a>
            @endforeach
            <div class="pt-3 border-t border-gray-100 flex flex-col gap-2">
                <a href="/login"    class="block px-3 py-2 text-sm font-medium text-center text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50">Masuk</a>
                <a href="/register" class="block px-3 py-2 text-sm font-medium text-center text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">Coba Gratis</a>
            </div>
    </div>
</header>
